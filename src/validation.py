...
from src.config import PETRI_NET_SCHEMA_PATH, OUTPUTS_DIR, INVALID_AUTO_REJECTED_DIR # Ensure OUTPUTS_DIR is imported
    ...
    try:
        ...
        for i, arc in enumerate(arcs):
            ...
            if arc_from not in all_defined_node_ids:
                return False, f"Arc {i} 'from' node '{arc_from}' not found in defined places or transitions."
            if arc_to not in all_defined_node_ids:
                return False, f"Arc {i} 'to' node '{arc_to}' not found in defined places or transitions."
            ...
            if (arc_from in places and arc_to in places) or \
               (arc_from in transitions and arc_to in transitions):
                return False, f"Arc {i} from '{arc_from}' to '{arc_to}' is invalid: Cannot connect same node types directly (e.g., place-to-place or transition-to-transition)."

        # Check initial marking consistency
        for place_id in initial_marking_places:
            if place_id not in places:
                return False, f"Initial marking for place '{place_id}' is invalid: Place ID not defined in 'places'."
        
        if arcs: 
            for p_id in places:
                if p_id not in nodes_in_arcs and p_id not in initial_marking_places:
                    if len(all_defined_node_ids) > 1: 
                        return False, f"Validation Error: Place '{p_id}' ('{places[p_id]}') is isolated (not part of any arc and not an initial marking)."
            for t_id in transitions:
                if t_id not in nodes_in_arcs:
                     if len(all_defined_node_ids) > 1: 
                        return False, f"Validation Error: Transition '{t_id}' ('{transitions[t_id]}') is isolated (not part of any arc)."
        elif not arcs and (len(places) > 1 or len(transitions) > 1 or (len(places) == 1 and len(transitions) == 1)):
            return False, "Validation Error: Multiple places/transitions exist but no arcs are defined to connect them."
        elif not arcs and (len(places) + len(transitions) > 1): 
             return False, "Validation Error: Multiple nodes exist but no arcs connect them."
...
def save_auto_rejected_sample(petri_json_data, scenario_text, reason, original_filename_base="unknown_candidate"):
    """Saves the auto-rejected (structurally invalid) Petri Net JSON to a dedicated directory."""
    try:
        timestamp = uuid.uuid4().hex[:8]
        # Sanitize reason for filename
        safe_reason = "".join(c if c.isalnum() else "_" for c in reason[:30]) # Limit length and make alphanumeric
        
        invalid_filename_base = f"{original_filename_base}_autorejected_{safe_reason}_{timestamp}"
        
        output_json_path = INVALID_AUTO_REJECTED_DIR / f"{invalid_filename_base}_petri.json"
        output_text_path = INVALID_AUTO_REJECTED_DIR / f"{invalid_filename_base}_text.txt"
        ...
        print(f"  --> Auto-rejected sample saved for review to: {INVALID_AUTO_REJECTED_DIR.resolve()}")
