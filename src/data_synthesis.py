...
def load_hand_made_sample(sample_id_str):
    ...
    try:
        ...
        with open(json_path, 'r', encoding='utf-8') as f: petri_json = json.load(f)
...
def _get_next_approved_folder_index(approved_dir: Path):
...
def save_final_sample_files(text_content, petri_json_data,
    ...
    try:
        with open(text_output_path, 'w', encoding='utf-8') as f: f.write(text_content)
        with open(json_output_path, 'w', encoding='utf-8') as f: json.dump(petri_json_data, f, indent=2)
...
def synthesize_paraphrases(original_text, original_petri_json, num_paraphrases=3, sample_base_name="paraphrase_source"):
...
"""
    response_content_str = get_llm_response(
        prompt_text=user_prompt, system_instruction=system_instruction,
        temperature=0.7, json_mode=False
    )
    if not response_content_str:
        print("Failed to get paraphrases from LLM."); return
    ...
    for i, paraphrase_text in enumerate(paraphrases_list):
        ...
        if paraphrase_text:
            ...
            paraphrase_candidate_base = f"{sample_base_name}_paraphrase{i+1}" # Unique base for this paraphrase
            save_final_sample_files(
                paraphrase_text, original_petri_json,
                SYNTHESIZED_APPROVED_DIR,
            ...
            generated_count += 1
            if generated_count >= num_paraphrases: break
    if generated_count == 0: print("No valid paraphrases extracted.")
    else: print(f"Saved {generated_count} paraphrased samples to numbered subfolders in {SYNTHESIZED_APPROVED_DIR.resolve()}.")
...
def build_few_shot_prompt_from_handmade(num_examples=2):
    ...
    if not isinstance(HAND_MADE_DIR, Path):
        print("Error: HAND_MADE_DIR is not a valid Path object in config."); return ""
    ...
    for text_file_path in sample_files:
        ...
        if text and petri_json:
            ...
            loaded_count += 1
            if loaded_count >= num_examples: break
    if loaded_count == 0: print(f"Warning: No hand-made samples found in {HAND_MADE_DIR} for few-shot examples.")
    else: print(f"Built few-shot prompt using {loaded_count} example(s).")
    return prompt_str
...
"""
    ...
    for i in range(num_new_samples):
        ...
        new_text, new_petri_json = generated_pair_dict.get("scenario_text"), generated_pair_dict.get("petri_net_json")
        if not all([new_text, isinstance(new_text, str), new_petri_json, isinstance(new_petri_json, dict)]):
            save_auto_rejected_sample(generated_pair_dict, new_text or "No text", "llm_malformed_output", candidate_id_base); continue
        ...
        if not is_structurally_valid:
            ...
            save_auto_rejected_sample(new_petri_json, new_text, validation_msg, candidate_id_base)
            continue
        ...
        try:
            ...
            with open(temp_json_path, 'w', encoding='utf-8') as f: json.dump(new_petri_json, f, indent=2)
        except Exception as e: print(f"  Warning: Could not save candidate text/JSON to review_temp: {e}"); continue
        ...
        target_parent_dir_for_sample = SYNTHESIZED_APPROVED_DIR if is_approved else SYNTHESIZED_REJECTED_MANUAL_DIR
        ...
        final_save_dir, final_filename_stem = save_final_sample_files(
            new_text, new_petri_json, target_parent_dir_for_sample, candidate_id_base, status_suffix_for_sample,
        ...
        for f_path in files_to_clean_in_review_temp:
            if f_path and f_path.exists() and f_path.parent == REVIEW_TEMP_DIR:
                try:
                    ...
                    print(f"  Warning: Could not delete temp review file {f_path.name}: {e}")
